<?php
/**
 * Report template for managed care tool
 *
 * @package 1.0.0
 */

echo do_shortcode( '[omda-nav subheading="Managed Care Assessment Results" linkto="submissions" button_txt="back"]' );
...
if ( ! $entry_id ) {
	...
	foreach ( $this->mc_metadata['sections'] as $section_key => $section ) {
...
<style>
	...
	.accordian-head {
		width: 100%;
		display: grid;
		grid-template-columns: 20fr 4fr 1fr;
		border: none;
		outline: none;
		margin: 0;
		padding: 0;
...
<script type="text/javascript">
	...
	const ctx = 'mcChart';
	const mcChart = new Chart(ctx, {
	type: 'bar',
	data: {
		labels: [
			['Clinical', 'Operations'],
			['Customer', 'Focus'],
			['Network', 'Management &', 'Marketing'],
